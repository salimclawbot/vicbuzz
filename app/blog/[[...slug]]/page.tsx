...
export default async function BlogPage({ params }: Props) {
  ...
  const faqSchema = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity:         {/* Social Share */}
        <div className="mt-8 pt-6 border-t border-gray-200">
          <p className="text-xs font-semibold uppercase tracking-widest text-[#FF6B6B] mb-3">Share this guide</p>
          <div className="flex gap-2">
            <button onClick={() => typeof window !== "undefined" && window.open(`https://twitter.com/intent/tweet?text=${encodeURIComponent(article.title)}&url=${encodeURIComponent(window.location.href)}`, "_blank")}
            ...
            <button onClick={() => typeof window !== "undefined" && window.open(`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(window.location.href)}`, "_blank")}
